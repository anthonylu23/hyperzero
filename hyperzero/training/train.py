...
def train_v1(
    ...
    *,
    ...
    for iteration in range(1, config.iterations + 1):
        ...
        evaluations = _evaluate_training_model(
            config,
            model,
            iteration=iteration,
            device=device,
...
def _training_step(
    ...
    *,
    ...
    model.train()
    boards, policies, values, legal_masks = _batch_tensors(examples, device)
...
def _evaluate_training_model(
    ...
    *,
    ...
    if config.eval_games_per_iteration == 0 or not config.eval_opponents:
        return {}
